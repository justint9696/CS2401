...
public class Calories2
{
	...
	public static void totalCalories(int [][] caloriesArray) // Calculates the total amount of calories for each day
	{
	...
	public static void maxCaloriesMeal(int [][] caloriesArray) // Calculates the highest number of documented calories consumed for each meal
	{
		...
		for(int i = 0;i < largestRow;i++)
		{
			...
			for(int j = 0;j < caloriesArray.length;j++)
			{
				if(caloriesArray[j].length < (i + 1)) continue;
				else if(caloriesArray[j][i] > max) max = caloriesArray[j][i];
			}
			System.out.println("Maximum Calories Consumed for Meal " + (i + 1) + ": " + max);
			index++;
		}
	}
